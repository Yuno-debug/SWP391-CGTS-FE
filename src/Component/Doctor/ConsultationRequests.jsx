...
const ConsultationRequests = () => {
  ...
  useEffect(() => {
    ...
    if (doctorId) {
      axios.get(`http://localhost:5200/api/Doctor/${doctorId}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem("token")}` }
      })
        .then((res) => {
          setDoctorName(res.data?.name || "Doctor Name Not Found");
        })
        .catch(() => {
          setDoctorName("N/A");
        });
  ...
  const handleBack = () => setSelectedChildId(null);
  ...
  const handleCreateResponse = (request) => openResponseModal(request);
  ...
  const handleSubmitResponse = async () => {
    ...
    try {
      await axios.post('http://localhost:5200/api/ConsultationResponse/create', payload, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      await fetchRequests();
      await fetchAllResponses();
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => {
      if (isDetailsModalOpen && detailsModalRef.current && !detailsModalRef.current.contains(e.target)) closeDetailsModal();
      if (isResponseModalOpen && responseModalRef.current && !responseModalRef.current.contains(e.target)) closeResponseModal();
      if (isViewResponseModalOpen && viewResponseModalRef.current && !viewResponseModalRef.current.contains(e.target)) closeViewResponseModal();
    };
  ...
  const getAvatarUrl = (child) => child.allergies || "https://placehold.co/50x50?text=No+Image";
  const getRelationshipLabel = (relationship) => (relationship === "D" ? "Dad" : relationship === "M" ? "Mom" : relationship);
  ...
  return (
    <div className="consultation-requests-container">
      ...
      {/* Nút Back được đặt ngay dưới page-header */}
      {selectedChildId && (
        <button className="back-button" onClick={handleBack}>Back</button>
      )}

      {/* Hide summary section when a child is selected */}
      {!selectedChildId && (
        <div className="summary-section">
          <div className="summary-card"><h4>Total Requests</h4><div className="summary-value">{totalRequests}</div></div>
          <div className="summary-card"><h4>Active Requests</h4><div className="summary-value">{activeRequests}</div></div>
          <div className="summary-card"><h4>Pending Requests</h4><div className="summary-value">{pendingRequests}</div></div>
          <div className="summary-card"><h4>Inactive Requests</h4><div className="summary-value">{inactiveRequests}</div></div>
        </div>
      )}
        ...
        <div className="addchild-child-list">
          {selectedChildId ? (
            children.filter(child => child.childId === selectedChildId).map((child) => (
              <div key={child.childId} className="addchild-child-card addchild-child-card-selected">
                <div className="addchild-child-card-content">
                  <img src={getAvatarUrl(child)} alt={`${child.name}'s Avatar`} className="addchild-child-card-avatar" onError={(e) => (e.target.src = "https://placehold.co/50x50?text=No+Image")} />
            ...
            childrenWithRequests.length > 0 ? (
              childrenWithRequests.map((child) => (
                <div key={child.childId} className="addchild-child-card" onClick={() => handleChildSelect(child.childId)}>
                  <div className="addchild-child-card-content">
                    <img src={getAvatarUrl(child)} alt={`${child.name}'s Avatar`} className="addchild-child-card-avatar" onError={(e) => (e.target.src = "https://placehold.co/50x50?text=No+Image")} />
                    <div className="addchild-child-card-name">{child.name}</div>
                  </div>
              ...
              <div className="addchild-empty-state">
                <img src="https://via.placeholder.com/300?text=No+Children" alt="No children" className="addchild-empty-illustration" />
            ...
            <table className="consultation-requests-table">
              ...
              <tbody>
                {currentRequests.length > 0 ? (
                  currentRequests.map((request, index) => {
                    ...
                    return (
                      <tr key={request.requestId || index} className="request-table-row">
                        ...
                        <td><div className="child-info"><span className="child-avatar">{childName?.[0] || 'C'}</span>{childName}</div></td>
                        <td><div className="request-description" title={request.description || 'N/A'}>{request.description || 'N/A'}</div></td>
                        <td>{request.requestDate ? new Date(request.requestDate).toLocaleDateString() : 'N/A'}</td>
                        <td><span className={`status-badge status-${request.status?.toLowerCase() || 'unknown'}`}>{request.status || 'N/A'}</span></td>
                        <td>{request.lastUpdated ? new Date(request.lastUpdated).toLocaleDateString() : 'N/A'}</td>
                        <td>
                          <div className="action-buttons">
                            <button className="create-response-button" onClick={() => handleCreateResponse(request)}>Create Response</button>
                            <button className="doctor-sidebar__submenu-item" onClick={() => handleViewResponse(request.requestId)}>
                              <span className="doctor-sidebar__menu-text">View Response</span>
                            </button>
                            <button className="details-button" onClick={() => openDetailsModal(request)}>Details</button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  <tr><td colSpan="7" style={{ textAlign: 'center' }}>No requests available for this child.</td></tr>
                )}
              </tbody>
            </table>
            <div className="pagination">
              <span className="pagination-info">Showing {indexOfFirstRequest + 1} to {Math.min(indexOfLastRequest, filteredRequests.length)} of {filteredRequests.length} entries</span>
              <div className="pagination-buttons">
                <button onClick={handlePrevPage} disabled={currentPage === 1} className={currentPage === 1 ? 'disabled' : ''}>Previous</button>
              ...
              return (
                <div key={request.requestId} className="recent-request-card">
                  <div className="recent-request-header">
                    ...
                    <span className={`status-badge status-${request.status?.toLowerCase() || 'unknown'}`}>{request.status || 'N/A'}</span>
                  </div>
      ...
      <div className="page-footer">
        <div className="footer-left"><p>Last Updated: {new Date().toLocaleString()}</p></div>
        ...
        <div className="modal-overlay">
          <div className="modal animate-modal-in" ref={detailsModalRef}>
            ...
            <div className="modal-content">
              <div className="modal-field"><label>Request ID</label><p>{selectedDetailsRequest.requestId || 'N/A'}</p></div>
              <div className="modal-field"><label>Child</label><p>{children.find(c => c.childId === selectedDetailsRequest.childId)?.name || 'Unknown Child'}</p></div>
              <div className="modal-field"><label>Description</label><p>{selectedDetailsRequest.description || 'N/A'}</p></div>
              <div className="modal-field"><label>Status</label><p>{selectedDetailsRequest.status || 'N/A'}</p></div>
              <div className="modal-field"><label>Request Date</label><p>{selectedDetailsRequest.requestDate ? new Date(selectedDetailsRequest.requestDate).toLocaleString() : 'N/A'}</p></div>
              <div className="modal-field"><label>Last Updated</label><p>{selectedDetailsRequest.lastUpdated ? new Date(selectedDetailsRequest.lastUpdated).toLocaleString() : 'N/A'}</p></div>
          ...
          <div className="modal animate-modal-in" ref={responseModalRef}>
            ...
            <div className="modal-content">
              <div className="modal-field">
                ...
                <input type="text" value={children.find(c => c.childId === selectedRequestForResponse.childId)?.name || 'N/A'} readOnly className="modal-input" />
              </div>
              <div className="modal-field">
              ...
              <div className="modal-field">
                <label>Status</label>
                <select value={responseStatus} onChange={(e) => setResponseStatus(e.target.value)} className="modal-select">
              ...
              <div className="modal-field">
                ...
                <input type="text" value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} placeholder="Enter diagnosis (optional)" className="modal-input" />
              </div>
              <div className="modal-buttons">
                <button onClick={handleSubmitResponse} className="modal-button submit">Submit</button>
                <button onClick={closeResponseModal} className="modal-button cancel">Cancel</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="modal-content">
              {viewResponseLoading ? (
                ...
                selectedRequestResponses.map((response, index) => (
                  <div key={response.responseId || index} className="response-item">
                    <div className="modal-field">
              ...
              <div className="modal-buttons">
                <button onClick={closeViewResponseModal} className="modal-button cancel">Close</button>
              </div>
            </div>
          </div>
        </div>
